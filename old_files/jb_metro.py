...
geojson_datasets = {
    f"{gtfs_prefix}stops_geojson":  stops_geojson,
    f"{gtfs_prefix}routes_geojson": routes_geojson,
}
    ...
    try:
        ...
        if dbcreds['dbms'] == "mongodb":
            ...
            for uri, gj in geojson_datasets.items():
                ...
                # this approach has bug
                #cursor.execute(
                #      f"INSERT INTO `{uri}` (uri, geojson) VALUES (%s, %s) AS new_row ON DUPLICATE KEY UPDATE geojson = new_row.geojson",
                #      (uri, json.dumps(gj, ensure_ascii=False))
                #  )
                cursor.execute(
